      ...
      <div className="w-full flex gap-12px">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-4px flex-y-center gap-8px">
            ...
            {!item.read && <div className="h-8px w-8px shrink-0 rounded-full bg-$ant-color-primary" title="未读" />}
          </div>
      ...
      <DarkModeContainer className="w-400px rounded-8px shadow-md">
        ...
        <div className="flex-y-center justify-between border-b border-$ant-color-border px-16px py-12px">
          <div className="flex-y-center gap-8px">
            ...
            {unreadCount > 0 && <ABadge count={unreadCount} overflowCount={99} showZero={false} />}
          </div>

          <div className="flex-y-center gap-8px">
            {unreadCount > 0 && (
              <AButton size="small" type="text" onClick={onMarkAllRead}>
                {t('notification.markAllRead')}
              </AButton>
            )}
            {notifications.length > 0 && (
              <AButton danger size="small" type="text" onClick={onClearAll}>
        ...
        <div className="max-h-500px overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            <List
              ...
              size="small"
              renderItem={item => <NotificationItemComponent item={item} onClick={onItemClick} onDelete={onDelete} />}
            />
          )}
        </div>
      </DarkModeContainer>
