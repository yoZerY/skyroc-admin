...
| `permissionSuperRole` | 静态权限校验中的超级角色标识。 |
...
动态菜单返回的 `routes` 会被归一成和静态菜单相同的 `GeneratedMenu` 与 `quickReferenceMenus`。关键点是：动态菜单只改变菜单、首页和授权数据来源，不会在运行时创建新的 React 页面组件。

因此后端返回的 `path` 仍然要对应前端已经存在的 TanStack Router 页面。否则菜单可以出现，但点击后会进入前端 404 或找不到页面。
